...
import {anthropicChat,parseJsonSafe,hasApiKey,setApiKey,clearApiKey,generateImage,hasKieKey,setKieKey,generateImageKie} from './api';
...
function App(){
  ...
  const renderTab=()=>{
    ...
    switch(tabIdx){
      case 0: return res?<div><div style={{padding:12,background:"#0a0c14",borderRadius:10,border:`1px solid ${brd}`,marginBottom:12}}><p style={{color:txt,fontSize:12,lineHeight:1.8}}>{r.ozet}</p></div>{generatedImageUrl&&<div style={{marginBottom:12}}><img src={generatedImageUrl} alt="AI görsel" style={{width:"100%",maxWidth:600,borderRadius:12,border:`1px solid ${brd}`}}/></div>}{!(hasApiKey()||hasKieKey())?<p style={{color:sub,fontSize:11,marginBottom:8}}>🖼️ Görsel için 🔑 menüsünden Claude veya Kie AI API anahtarı girin.</p>:<><div style={{display:"flex",flexWrap:"wrap",gap:8,marginBottom:8}}>{hasApiKey()&&<button onClick={async()=>{const prompt=r.mjPrompt||r.ozet||"";if(!prompt)return;setImageLoad(true);setImageError(null);const out=await generateImage(prompt);setImageLoad(false);if(out.ok){setGeneratedImageUrl(out.url);}else setImageError(out.error);}} disabled={imageLoad} style={{padding:"10px 20px",borderRadius:10,background:imageLoad?"#1c1f33":"linear-gradient(135deg,#10a37f,#0d8a6a)",color:imageLoad?sub:"#fff",fontWeight:700,border:"none",cursor:"pointer"}}>{imageLoad?"⏳ Üretiliyor...":"🖼️ Görsel üret (Claude)"}</button>}{hasKieKey()&&<button onClick={async()=>{const prompt=r.mjPrompt||r.ozet||"";if(!prompt)return;setImageLoad(true);setImageError(null);const out=await generateImageKie(prompt);setImageLoad(false);if(out.ok){setGeneratedImageUrl(out.url);}else setImageError(out.error);}} disabled={imageLoad} style={{padding:"10px 20px",borderRadius:10,background:imageLoad?"#1c1f33":"linear-gradient(135deg,#d4a853,#b8892a)",color:imageLoad?sub:"#080810",fontWeight:700,border:"none",cursor:"pointer"}}>{imageLoad?"⏳ Üretiliyor...":"🖼️ Görsel üret Kie AI"}</button>}</div>{imageError&&<p style={{color:"#ff8888",fontSize:12,marginTop:8}}>{imageError}</p>}</>}</div>:<div style={{textAlign:"center",padding:24}}><p style={{color:sub,marginBottom:12}}>Önce &quot;AI ile Üret&quot; ile konsept oluşturun.</p><button onClick={genAI} disabled={load} style={{padding:"12px 24px",borderRadius:10,background:load?"#1c1f33":`linear-gradient(135deg,${gold},#b8892a)`,color:load?sub:"#080810",fontWeight:700,border:"none",cursor:"pointer"}}>{load?t.generating:t.generate}</button></div>;
      ...
      case 11: return res?<div style={{padding:12}}><p style={{color:goldL,fontSize:11,marginBottom:8}}>🖼️ Görsel prompt</p><p style={{color:txt,fontSize:12,marginBottom:12}}>{r.mjPrompt||r.ozet}</p>{!(hasApiKey()||hasKieKey())?<p style={{color:sub,fontSize:11}}>Görsel için 🔑 menüsünden Claude veya Kie AI API anahtarı girin.</p>:<><div style={{display:"flex",flexWrap:"wrap",gap:8,marginBottom:8}}>{hasApiKey()&&<button onClick={async()=>{const prompt=r.mjPrompt||r.ozet||"";if(!prompt)return;setImageLoad(true);setImageError(null);const out=await generateImage(prompt);setImageLoad(false);if(out.ok){setGeneratedImageUrl(out.url);}else setImageError(out.error);}} disabled={imageLoad} style={{padding:"10px 20px",borderRadius:10,background:imageLoad?"#1c1f33":"linear-gradient(135deg,#10a37f,#0d8a6a)",color:imageLoad?sub:"#fff",fontWeight:700,border:"none",cursor:"pointer"}}>🖼️ Görsel üret (Claude)</button>}{hasKieKey()&&<button onClick={async()=>{const prompt=r.mjPrompt||r.ozet||"";if(!prompt)return;setImageLoad(true);setImageError(null);const out=await generateImageKie(prompt);setImageLoad(false);if(out.ok){setGeneratedImageUrl(out.url);}else setImageError(out.error);}} disabled={imageLoad} style={{padding:"10px 20px",borderRadius:10,background:imageLoad?"#1c1f33":"linear-gradient(135deg,#d4a853,#b8892a)",color:imageLoad?sub:"#080810",fontWeight:700,border:"none",cursor:"pointer"}}>🖼️ Görsel üret Kie AI</button>}</div>{imageError&&<p style={{color:"#ff8888",fontSize:12,marginTop:8}}>{imageError}</p>}{generatedImageUrl&&<div style={{marginTop:12}}><img src={generatedImageUrl} alt="Mimari görsel" style={{width:"100%",maxWidth:640,borderRadius:12,border:`1px solid ${brd}`}}/></div>}</>}</div>:null;
  ...
  return(<div style={{minHeight:"100vh",background:bg,color:txt,fontFamily:"system-ui,-apple-system,Segoe UI,Roboto,sans-serif"}} className="architect-app">
    ...
    {showApiKeyModal&&<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.85)",zIndex:999,display:"flex",alignItems:"center",justifyContent:"center"}} onClick={()=>setShowApiKeyModal(false)}><div style={{background:card,border:`1px solid ${brd}`,borderRadius:12,padding:20,minWidth:340}} onClick={e=>e.stopPropagation()}>
      <h3 style={{margin:"0 0 12px",color:goldL,fontSize:14}}>🔑 API anahtarları</h3>
      <p style={{color:sub,fontSize:11,marginBottom:10}}>Claude: metin + SVG görsel. Kie AI: fotoğraf kalitesinde görsel (gpt4o-image). Tarayıcıda saklanır.</p>
      <label style={{display:"block",color:sub,fontSize:10,marginBottom:4}}>Anthropic (Claude) — sk-ant-...</label>
      <input type="password" value={apiKeyInput} onChange={e=>setApiKeyInput(e.target.value)} placeholder="Claude API anahtarı" style={{width:"100%",padding:"9px 12px",borderRadius:8,border:`1px solid ${brd}`,background:"#0a0c14",color:txt,fontSize:12,outline:"none",marginBottom:12}} />
      <label style={{display:"block",color:sub,fontSize:10,marginBottom:4}}>Kie AI (görsel) — kie.ai/api-key</label>
      <input type="password" value={kieKeyInput} onChange={e=>setKieKeyInput(e.target.value)} placeholder="Kie AI API anahtarı" style={{width:"100%",padding:"9px 12px",borderRadius:8,border:`1px solid ${brd}`,background:"#0a0c14",color:txt,fontSize:12,outline:"none",marginBottom:12}} />
      <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
        <button onClick={()=>{if(apiKeyInput.trim()){setApiKey(apiKeyInput.trim());setApiKeyInput("");}if(kieKeyInput.trim()){setKieKey(kieKeyInput.trim());setKieKeyInput("");}setShowApiKeyModal(false);forceRefresh(n=>n+1);}} style={{padding:"8px 16px",borderRadius:8,background:gold,color:"#080810",fontWeight:700,border:"none",cursor:"pointer"}}>Kaydet</button>
        <button onClick={()=>{clearApiKey();setApiKeyInput("");setShowApiKeyModal(false);forceRefresh(n=>n+1);}} style={{padding:"8px 16px",borderRadius:8,background:"#1a0808",border:"1px solid #3a1010",color:"#ff8888",cursor:"pointer"}}>Claude kaldır</button>
        <button onClick={()=>{setKieKey("");setKieKeyInput("");setShowApiKeyModal(false);forceRefresh(n=>n+1);}} style={{padding:"8px 16px",borderRadius:8,background:"#1a0808",border:"1px solid #3a1010",color:"#ff8888",cursor:"pointer"}}>Kie kaldır</button>
